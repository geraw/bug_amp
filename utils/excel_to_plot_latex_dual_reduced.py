...
def plot_graph(x, ys, yerrs, series_labels, png_path, title):
    ...
    for i, label in enumerate(series_labels):
        plt.errorbar(x, ys[i], yerr=yerrs[i], label=label, fmt=f'-{markers[i % len(markers)]}', capsize=3)
    plt.ylim(0, 1)
    plt.grid(True)
    plt.legend()
    plt.title(title)
...
"""
    ...
    for i, label in enumerate(series_labels):
        ...
        marker = latex_markers[i % len(latex_markers)]
        body += f"% Plot for {escaped_label}\n"
        body += f"\\addplot+[mark={marker}, error bars/.cd, y dir=both, y explicit] coordinates {{\n"
...
def main():
    ...
    # Keep only: Ens and BF (no Classifier, no MLP)
    fifth_best_indices = [(3, 3), (12, 12)]
    fifth_labels = ["Ens_5^th", "BF_5^th"]

    tenth_best_indices = [(4, 4), (13, 13)]
    tenth_labels = ["Ens_10^th", "BF_10^th"]
