    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                ...
                color = 'green' if p_val < alpha else 'red'
                matrix[i][j] = (color, f"{p_val:.3f}")
    return matrix
    ...
    for i, row in enumerate(matrix):
        ...
        for j, cell in enumerate(row):
            if cell == 'diag':
                ...
                color, pval = cell
                tcolor = "lightgreen" if color == 'green' else "lightred"
                # tikz_box = rf"\tikz[baseline=(char.base)]{{\node[fill={tcolor},rounded corners=0.5mm,inner sep=1pt] (char) {{{pval}}};}}"
                tikz_box = rf"\tikz[baseline=(char.base)]{{\node[fill={tcolor},rounded corners=0.5mm,inner sep=4pt, font=\normalsize] (char) {{{pval}}};}}"
